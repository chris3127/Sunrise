...
API_KEY_GEOCODE = os.getenv("API_KEY_GEOCODE") #API key is obscured for security

class Lat_Long_Address:
    '''
    This class calls the LocationIQ Geocode API to get location data for a specific address.
    '''
    ...
    def display_address(self):
        '''
        Returns the complete address that has been used to find the latitude and longitude as a string.
            Param self: Lat_Long_Address
            Type return: String
        '''
        address_data = self.response.json()
...
class Sunrise_Time:
    ...
    def sunrise_output(self):
        '''
        This method takes the location sunrise data, parses it and renders it to HTML.
...
@app.route('/', methods=['GET', 'POST']) #First and second route, get HTML form, post user input
def login():
    form = LoginForm()
...
@app.route('/sunrise', methods=['GET']) #Third route, get HTML and render data for user view
def conversion():
    '''
...
def sunrise(lat, long):
    ...
    Gets local time zone from latitude and longitude, localises sunrise time for local time zone.
